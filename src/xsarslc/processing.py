...
from sar_slc.tools import xtiling, xndindex
...
def compute_subswath_xspectra(dt, **kwargs):
    ...
    import datatree
    import cartopy
    from slc_sar.tools import netcdf_compliant
...
def compute_subswath_intraburst_xspectra(dt, tile_width={'sample':20.e3, 'line':20.e3}, tile_overlap={'sample':10.e3, 'line':10.e3}, **kwargs):
    ...
    commons = {'radar_frequency':float(dt['image']['radarFrequency']), 'mean_incidence':float(dt['image']['incidenceAngleMidSwath']), 'azimuth_time_interval':float(dt['image']['azimuthTimeInterval'])}
    xspectra = list()
    for b in range(dt['bursts'].sizes['burst']):
        ...
        burst_xspectra = tile_burst_to_xspectra(burst, dt['geolocation_annotation'], dt['orbit'], tile_width, tile_overlap, **kwargs)
    ...
    xspectra = xr.combine_by_coords([x[{'freq_sample':slice(None,Nfreq_min)}] for x in xspectra], combine_attrs='drop_conflicts') # rearange xs on burst
...
def compute_subswath_interburst_xspectra(dt, tile_width={'sample':20.e3, 'line':20.e3}, tile_overlap={'sample':10.e3, 'line':10.e3}, **kwargs):
    ...
    commons = {'azimuth_steering_rate':dt['image']['azimuthSteeringRate'].item(),'mean_incidence':float(dt['image']['incidenceAngleMidSwath']), 'azimuth_time_interval':float(dt['image']['azimuthTimeInterval'])}
    xspectra = list()
    for b in range(dt['bursts'].sizes['burst']-1):
        ...
        interburst_xspectra = tile_bursts_overlap_to_xspectra(burst0, burst1, dt['geolocation_annotation'], tile_width, tile_overlap, **kwargs)
...
def tile_burst_to_xspectra(burst, geolocation_annotation, orbit, tile_width, tile_overlap, lowpass_width = {'sample':1000., 'line':1000.}, periodo_width = {'sample':2000., 'line':4000.}, periodo_overlap = {'sample':1000., 'line':2000.}, **kwargs):
    """
    ...
    if tile_overlap in (0.,None):
        ...
        noverlap = {d:int(np.rint(tile_overlap[d]/spacing[d])) for d in tile_width.keys()} # np.rint is important for homogeneity of point numbers between bursts
    ...
    middle_lons = FullResolutionInterpolation(middle_line, middle_sample, 'longitude', geolocation_annotation, azitime_interval)
    middle_lats = FullResolutionInterpolation(middle_line, middle_sample, 'latitude', geolocation_annotation, azitime_interval)
    ...
    azitime_interval = burst.attrs['azimuth_time_interval']
    corner_lons = FullResolutionInterpolation(corner_line, corner_sample, 'longitude', geolocation_annotation, azitime_interval).unstack(dim=['flats', 'flatl']).rename('corner_longitude')
    corner_lats = FullResolutionInterpolation(corner_line, corner_sample, 'latitude', geolocation_annotation, azitime_interval).unstack(dim=['flats', 'flatl']).rename('corner_latitude')
    corner_incs = FullResolutionInterpolation(corner_line, corner_sample, 'incidenceAngle', geolocation_annotation, azitime_interval).unstack(dim=['flats', 'flatl'])
    corner_slantTimes = FullResolutionInterpolation(corner_line, corner_sample, 'slantRangeTime', geolocation_annotation, azitime_interval).unstack(dim=['flats', 'flatl'])
    ...
    for i in xndindex(tiles_sizes):
        ...
        if 'landmask' in kwargs:
            tile_lons = [float(corner_lons[i][{'corner_line':j, 'corner_sample':k}]) for j,k in [(0,0), (1,0), (1,1), (0,1), (0,0)]]
            tile_lats = [float(corner_lats[i][{'corner_line':j, 'corner_sample':k}]) for j,k in [(0,0), (1,0), (1,1), (0,1), (0,0)]]
        ...
        if water_only:
            ...
            synthetic_duration = celerity*mean_slant_range/(2*burst.attrs['radar_frequency']*mean_velocity*azimuth_spacing)
            ...
            xspecs = compute_intraburst_xspectrum(mod, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration, nperseg = nperseg_periodo, noverlap = noverlap_periodo, **kwargs)
    ...
    if xs: # at least one existing xspectra has been calculated
        ...
        xs = xr.combine_by_coords([x[{'freq_sample':slice(None,Nfreq_min)}].expand_dims(['tile_sample', 'tile_line']) for x in xs], combine_attrs='drop_conflicts')
        ...
        xs = xr.merge([xs, taus.to_dataset(), cutoff.to_dataset(), corner_lons.to_dataset(), corner_lats.to_dataset()], combine_attrs='drop_conflicts')
        xs = xs.assign_coords({'longitude':middle_lons, 'latitude':middle_lats}) # This line also ensures adding line/sample coordinates too !! DO NOT REMOVE
...
def compute_intraburst_xspectrum(slc, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration, azimuth_dim='line', nperseg={'sample':512, 'line':512}, noverlap={'sample':256, 'line':256}, **kwargs):
    ...
    k_rg = xr.DataArray(np.fft.rfftfreq(nperseg[range_dim], ground_range_spacing/(2*np.pi)), dims='freq_'+range_dim, name = 'k_rg', attrs={'long_name':'wavenumber in range direction', 'units':'rad/m'})
...
def compute_looks(slc, azimuth_dim, synthetic_duration, nlooks=3, look_width=0.2, look_overlap=0., look_window=None, **kwargs):
    """
    ...
    for l in xndindex(looks_sizes):
        ...
        look = xrft.fft(look, dim=slc.dims, detrend='constant', true_phase=True, true_amplitude=True, shift=False) # shift=False is to keep zero at begnining for easier selection of half the spectrum
        look.data = np.fft.fftshift(look.data, axes=look.get_axis_num(freq_azi_dim)) # fftshifting azimuthal wavenumbers
        looks_spec.append(look[{freq_rg_dim:slice(None,look.sizes[freq_rg_dim]//2+1)}]) # Only half of the spectrum is kept
...
def tile_bursts_overlap_to_xspectra(burst0, burst1, geolocation_annotation, tile_width, tile_overlap, lowpass_width = {'sample':1000., 'line':1000.}, periodo_width = {'sample':2000., 'line':1400.}, periodo_overlap = {'sample':1000., 'line':700.}, **kwargs):
    """
    ...
    # ---------Computing quantities at tile corner locations  --------------------------
    tiles_corners = get_corner_tile(tiles_index)# Having variables below at corner positions is sufficent for further calculations (and save memory space)
    corner_sample = burst['sample'][{'sample':tiles_corners['sample']}]
    corner_sample =corner_sample.stack(flats=corner_sample.dims)
    corner_line = burst['line'][{'line':tiles_corners['line']}]
    corner_line = corner_line.stack(flatl=corner_line.dims)
    ...
    for i in xndindex(tiles_sizes):
        ...
        if water_only:
            ...
            if np.any([sub0.sizes[d]<nperseg_periodo[d] for d in ['line', 'sample']]):
                raise ValueError('periodo_width ({}) is too large compared to available data (line : {} m, sample : {} m).'.format(periodo_width, sub0.sizes['line']*azimuth_spacing, sub0.sizes['sample']*ground_spacing))
            ...
            xspecs = compute_interburst_xspectrum(mod0**2, mod1**2, mean_incidence, slant_spacing, azimuth_spacing, nperseg = nperseg_periodo, noverlap = noverlap_periodo, **kwargs)
            xspecs_m = xspecs.mean(dim=['periodo_line', 'periodo_sample'], keep_attrs=True) # averaging all the periodograms in each tile
            # xs[tuple(i.values())] = xspecs_m
            xs.append(xspecs_m)
            # ------------- tau ------------------
...
def compute_interburst_xspectrum(mod0, mod1, mean_incidence, slant_spacing, azimuth_spacing, azimuth_dim='line', nperseg={'sample':512, 'line':None}, noverlap={'sample':256,'line':0}):
    ...
    for i in xndindex(periodo_sizes):
        ...
        xspecs = xr.DataArray(np.fft.fft2(image1)*np.conj(np.fft.fft2(image0)), dims=['freq_'+d for d in image0.dims])
        xspecs = xspecs[{freq_rg_dim:slice(None, xspecs.sizes[freq_rg_dim]//2+1)}] # keeping only half of the wavespectrum (positive wavenumbers)
        xspecs.data = np.fft.fftshift(xspecs.data, axes=xspecs.get_axis_num(freq_azi_dim)) # fftshifting azimuthal wavenumbers
        out[tuple(i.values())] = xspecs
    ...
    # dealing with wavenumbers
    ground_range_spacing = slant_spacing/np.sin(np.radians(out.mean_incidence))
    k_rg = xr.DataArray(np.fft.rfftfreq(nperseg[range_dim], ground_range_spacing/(2*np.pi)), dims=freq_rg_dim, name = 'k_rg', attrs={'long_name':'wavenumber in range direction', 'units':'rad/m'})
    k_az = xr.DataArray(np.fft.fftshift(np.fft.fftfreq(nperseg[azimuth_dim], azimuth_spacing/(2*np.pi))), dims='freq_'+azimuth_dim, name = 'k_az', attrs={'long_name':'wavenumber in azimuth direction', 'units':'rad/m'})
    # out = out.assign_coords({'k_rg':k_rg, 'k_az':k_az}).swap_dims({'freq_'+range_dim:'k_rg', 'freq_'+azimuth_dim:'k_az'})
    out = xr.merge([out, k_rg.to_dataset(), k_az.to_dataset()], combine_attrs='drop_conflicts') # Adding .to_dataset() ensures promote_attrs=False
    out.attrs.update({'periodogram_nperseg_'+range_dim:nperseg[range_dim], 'periodogram_nperseg_'+azimuth_dim:nperseg[azimuth_dim], 'periodogram_noverlap_'+range_dim:noverlap[range_dim], 'periodogram_noverlap_'+azimuth_dim:noverlap[azimuth_dim]})
    return out
...
def symmetrize_xspectrum(xs, dim_range = 'k_rg', dim_azimuth = 'k_az'):
    ...
    if (dim_range not in xs.coords) or (dim_azimuth not in xs.coords):
        raise ValueError('Symmetry can not be applied because {} or {} do not have coordinates. Swap dimensions prior to symmetrization.'.format(dim_range, dim_azimuth))
...
# def symmetrize_xspectrum_old_and_bugged(xs, dim_range = 'k_rg', dim_azimuth = 'k_az'):
#     """
#     Symmetrize half-xspectrum around origin point. xspectrum is assumed to contain only positive wavenumbers in range.
...
#     if (dim_range not in xs.coords) or (dim_azimuth not in xs.coords):
#         raise ValueError('Symmetry can not be applied because {} or {} do not have coordinates. Swap dimensions prior to symmetrization.'.format(dim_range, dim_azimuth))
#     mirror = np.conj(xs[{dim_range:slice(None,0,-1), dim_azimuth:slice(1,None)}])
#     mirror = mirror.assign_coords({dim_range:-mirror[dim_range], dim_azimuth:-mirror[dim_azimuth]})
#     res = xr.concat([mirror, xs], dim=dim_range)[{dim_range:slice(None,-1)}]
#     if not xs.sizes['k_az']%2:# even number of point
#         res[{dim_azimuth:0}].data = res[{dim_azimuth:-1}]
#     res = res[{dim_azimuth:slice(None,-1)}]
#     return res
