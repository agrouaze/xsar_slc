...
def compute_midburst_azimuthtime(ds, azimuth_time_interval):
    ...
    midburstati = ds['sensingTime'] + lpb / 2 * np.timedelta64(int(azimuth_time_interval * 1e12), 'ps').astype(
        '<m8[ns]')
...
def compute_Doppler_rate(orbit, azimuth_steering_rate, radar_frequency):
    ...
    vs = np.sqrt(orbit['velocity_x'] ** 2 + orbit['velocity_y'] ** 2 + orbit['velocity_z'] ** 2).rename(
...
def compute_FMrate(FMrate, midburst_azimuth_time, slant_range_time):
    ...
    """
    iazi = (np.abs(midburst_azimuth_time - FMrate['azimuthTime'])).argmin(
        dim='azimuthTime')  # index of nearest midburst azimuth time
    ...
    ka = xr.apply_ufunc(eval_poly, poly, slant_range_time, t0, input_core_dims=[[], ['sample'], []], vectorize=True,
                        output_core_dims=[['sample']])
    ka.attrs.update({'units': 'Hz/s', 'long_name': 'Doppler_FMrate'})
    return ka


def compute_Doppler_centroid_rate(orbit, azimuth_steering_rate, radar_frequency, FMrate, midburst_azimuth_time,
                                  slant_range_time):
    """
...
def compute_DopplerCentroid_frequency(DopplerEstimate, midburst_azimuth_time, slant_range_time):
    ...
    Args:
        ...
        slant_range_time (xarray.DataArray) : array of slant range time
    """
    iazi = (np.abs(midburst_azimuth_time - DopplerEstimate['azimuthTime'])).argmin(
        dim='azimuthTime')  # index of nearest midburst azimuth time
    ...
    fnc = xr.apply_ufunc(eval_poly, poly, slant_range_time, t0, input_core_dims=[[], ['sample'], []], vectorize=True,
                         output_core_dims=[['sample']])
